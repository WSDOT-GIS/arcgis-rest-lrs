...
export interface IGeometryToMeasureValues {
  tolerance?: number;
  temporalViewDate?: Date
}

export class GeometryToMeasureControl implements IGeometryToMeasureValues {
  ...
  constructor(
    ...
    this.form.onsubmit = (e) => {
      const { tolerance, temporalViewDate } = self;
      const outEvent = new CustomEvent<IGeometryToMeasureValues>("g2mstarted", {
        detail: { tolerance, temporalViewDate }
      });
      self.form.dispatchEvent(outEvent);
      // try {
