...
import {
  GeometryToMeasureControl,
  MeasureToGeometryControl,
  IGeometryToMeasureValues
} from "@wsdot/crab";
...
export function setupCrab(
  ...
  window.addEventListener("mapload", e => {
    ...
    const g2mForm = new GeometryToMeasureControl();
    g2mForm.tolerance = 50;
    crabUI.appendChild(g2mForm.form);

    g2mForm.form.addEventListener("g2mstarted", evt => {
      const { tolerance, temporalViewDate } = (evt as CustomEvent<
        IGeometryToMeasureValues
      >).detail;
    });
